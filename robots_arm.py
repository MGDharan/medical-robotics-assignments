...
# Discover joints dynamically
arm_joints = []      # moveable arm joints (indices + names)
finger_joints = []   # finger/gripper joints
ee_link = 10
for _ji in range(bb.getNumJoints(robot)):
    jname, jtype, jlimits = bb.getJointInfo(robot, _ji)
    if jtype != 'fixed':
        if 'finger' in jname.lower():
            finger_joints.append((_ji, jname, jlimits))
        else:
            arm_joints.append((_ji, jname, jlimits))

FINGER_OPEN   = 0.04
FINGER_CLOSED = 0.0

def set_fingers(val):
    for ji, _, _ in finger_joints:
...
# Caster rollers under the base (visible wheels)
for cx, cy in [(-0.30,-0.30),(-0.30,0.30),(0.30,-0.30),(0.30,0.30)]:
    # wheel fork / housing
    _mk(robo_parts, [0.025, 0.025, 0.018], [RBX+cx, RBY+cy, 0.018], 0x333333)
    # roller wheel (dark rubber)
    _mk(robo_parts, [0.022, 0.035, 0.022], [RBX+cx, RBY+cy, 0.022], 0x1a1a1a)
    # axle pin
    _mk(robo_parts, [0.005, 0.04, 0.005], [RBX+cx, RBY+cy, 0.022], 0x888888)

# Main platform body
_mk(robo_parts, [0.35,0.35,0.06], [RBX,RBY,0.10], 0x1c3f6e)
# Platform edge trim
for sign in [-1, 1]:
    _mk(robo_parts, [0.36, 0.012, 0.065], [RBX, RBY+sign*0.35, 0.10], 0x142e55)
    _mk(robo_parts, [0.012, 0.35, 0.065], [RBX+sign*0.35, RBY, 0.10], 0x142e55)
# Cross markers on top
_mk(robo_parts, [0.33,0.018,0.005], [RBX,RBY,0.165], 0x00aaff)
_mk(robo_parts, [0.018,0.33,0.005], [RBX,RBY,0.165], 0x00aaff)
_mk(robo_parts, [0.026,0.026,0.032], [RBX+0.28,RBY,0.17], 0x00ff88)
...
cube_pos = list(LOC_A)  # track cube position
...
# ── Door panel (lead-lined hospital door) ─────────────────────
IR_DX = IR_CX - IRL/2 - IR_WALL_T + 0.05   # just inside the frame
# main panel
bb.createBody(shape="box", halfExtent=[0.03, IR_DOOR_W/2-0.06, IR_DOOR_H/2],
    position=[IR_DX, IR_CY, IR_DOOR_H/2], color=0x607888, mass=0)
# lower panel inset
bb.createBody(shape="box", halfExtent=[0.005, IR_DOOR_W/2-0.14, 0.45],
    position=[IR_DX+0.03, IR_CY, 0.55], color=0x506878, mass=0)
# upper observation window (leaded glass)
bb.createBody(shape="box", halfExtent=[0.005, 0.22, 0.28],
    position=[IR_DX+0.032, IR_CY, 2.0], color=0xb0d0e8, mass=0)
# window frame
for wfy in [-0.23, 0.23]:
    bb.createBody(shape="box", halfExtent=[0.008, 0.015, 0.29],
        position=[IR_DX+0.032, IR_CY+wfy, 2.0], color=0x485868, mass=0)
for wfz in [1.71, 2.29]:
    bb.createBody(shape="box", halfExtent=[0.008, 0.23, 0.015],
        position=[IR_DX+0.032, IR_CY, wfz], color=0x485868, mass=0)
# push plate
bb.createBody(shape="box", halfExtent=[0.006, 0.04, 0.10],
    position=[IR_DX+0.035, IR_CY+IR_DOOR_W/2-0.20, 1.05], color=0xc0c0c0, mass=0)
# handle
bb.createBody(shape="box", halfExtent=[0.02, 0.008, 0.06],
    position=[IR_DX+0.05, IR_CY+IR_DOOR_W/2-0.20, 1.05], color=0xa0a0a0, mass=0)
# radiation warning sign on door
bb.createBody(shape="box", halfExtent=[0.005, 0.10, 0.10],
    position=[IR_DX+0.033, IR_CY-0.05, 1.55], color=0xffcc00, mass=0)
bb.createBody(shape="box", halfExtent=[0.006, 0.06, 0.06],
    position=[IR_DX+0.035, IR_CY-0.05, 1.55], color=0x222222, mass=0)
...
# ── Double door panels (reception entrance) ───────────────────
REC_DX = REC_CX + REC_L/2 + REC_WT - 0.05   # just inside frame
half_panel = REC_DOOR_W/4 - 0.03            # half of one leaf width
# Left door leaf
bb.createBody(shape="box", halfExtent=[0.03, half_panel, REC_DOOR_H/2],
    position=[REC_DX, REC_CY - REC_DOOR_W/4 - 0.01, REC_DOOR_H/2], color=0x7a9e7e, mass=0)
# Right door leaf
bb.createBody(shape="box", halfExtent=[0.03, half_panel, REC_DOOR_H/2],
    position=[REC_DX, REC_CY + REC_DOOR_W/4 + 0.01, REC_DOOR_H/2], color=0x7a9e7e, mass=0)
# Centre gap strip (where doors meet)
bb.createBody(shape="box", halfExtent=[0.035, 0.012, REC_DOOR_H/2],
    position=[REC_DX, REC_CY, REC_DOOR_H/2], color=0x608868, mass=0)
# Glass windows (upper portion of each leaf)
for wpy in [-REC_DOOR_W/4-0.01, REC_DOOR_W/4+0.01]:
    bb.createBody(shape="box", halfExtent=[0.005, half_panel-0.06, 0.35],
        position=[REC_DX-0.032, REC_CY+wpy, 2.05], color=0xc8e0d8, mass=0)
    # window frame
    for wey in [wpy-(half_panel-0.05), wpy+(half_panel-0.05)]:
        bb.createBody(shape="box", halfExtent=[0.006, 0.012, 0.36],
            position=[REC_DX-0.032, REC_CY+wey, 2.05], color=0x5a8a6a, mass=0)
    for wez in [1.69, 2.41]:
        bb.createBody(shape="box", halfExtent=[0.006, half_panel-0.05, 0.012],
            position=[REC_DX-0.032, REC_CY+wpy, wez], color=0x5a8a6a, mass=0)
# Push plates (one per leaf)
for ppy in [-REC_DOOR_W/4+0.15, REC_DOOR_W/4-0.15]:
    bb.createBody(shape="box", halfExtent=[0.006, 0.04, 0.10],
        position=[REC_DX-0.035, REC_CY+ppy, 1.05], color=0xc0c0c0, mass=0)
    # handle bar
    bb.createBody(shape="box", halfExtent=[0.02, 0.008, 0.06],
        position=[REC_DX-0.055, REC_CY+ppy, 1.05], color=0xa0a0a0, mass=0)
# Kick plates at bottom of each leaf
for kpy in [-REC_DOOR_W/4-0.01, REC_DOOR_W/4+0.01]:
    bb.createBody(shape="box", halfExtent=[0.005, half_panel-0.02, 0.15],
        position=[REC_DX-0.032, REC_CY+kpy, 0.15], color=0xaaaaaa, mass=0)
...
# ============================================================
# ROBOT DOG  (Spot URDF — welcome dog in reception, stationary)
# ============================================================
# Place near the reception desk entrance, facing patients
SPOT_X = REC_CX + REC_L/2 - 1.5  # near the doorway
SPOT_Y = REC_CY - 1.0
spot_orn = bb.getQuaternionFromEuler([0, 0, math.pi])  # face toward patients
...
# Gentle stationary gait (smaller swing so it looks like a happy trot in place)
SPOT_TAU = 12
SPOT_HIP_BASE    = 0.8
SPOT_KNEE_BASE   = -1.6
SPOT_HIP_SWING   = 0.25
...
def animate_spot_dog():
    """Animate Spot's legs with a gentle walking gait in place."""
    global spot_step_i
    spot_step_i += 1
...
bb.addDebugSlider('base_Speed', 0.02, 0.002, 0.08, 0.002)
...
bb.addDebugSlider('arm_Speed', 2.0, 0.5, 5.0, 0.5)
...
def arm_home():
    for idx, (ji, jn, jlim) in enumerate(arm_joints):
        val = HOME_JP[idx] if idx < len(HOME_JP) else sum(jlim)/2
        bb.setJointMotorControl(robot, ji, targetPosition=val)
...
def ik_move(px, py, pz):
    ...
    for idx, (ji, jn, jlim) in enumerate(arm_joints):
        if idx < len(jp):
            bb.setJointMotorControl(robot, ji, targetPosition=jp[idx])
...
# Smooth base movement state
base_actual_x = float(RBX)   # where the base currently IS
base_actual_y = float(RBY)
BASE_MOVE_SPEED = 0.006       # metres per sim frame (smooth glide)
...
while True:
    ...
    # Robot base + tray (smooth glide on rollers toward slider target)
    dx_base = cur_base_x - base_actual_x
    dy_base = cur_base_y - base_actual_y
    dist_base = math.sqrt(dx_base*dx_base + dy_base*dy_base)
    if dist_base > 0.002:
        # clamp step to BASE_MOVE_SPEED so it glides, never teleports
        step_len = min(bb.readDebugParameter('base_Speed'), dist_base)
        base_actual_x += (dx_base / dist_base) * step_len
        base_actual_y += (dy_base / dist_base) * step_len
        reposition_group(robo_parts, base_actual_x, base_actual_y, RBX, RBY)
        try:
            bb.resetBasePose(robot,
                [base_actual_x, base_actual_y, RBZ], robo_orn)
        except: pass
    ...
    if new_reset_btn > old_reset_btn:
        base_dx = base_actual_x - RBX
        base_dy = base_actual_y - RBY
    ...
    if auto_running and seq is not None:
        ...
        if label != prev_label:
            ...
            try:
                ...
                status_txt = bb.createDebugText(
                    label,
                    [base_actual_x, base_actual_y-0.65, 1.55],
        ...
        if stype == "action":
            ...
            arm_spd = bb.readDebugParameter('arm_Speed')
            scaled_hold = max(1, int(hold / arm_spd))
            if step_timer == 1:
                action_fn()
        ...
        if manual_moving and manual_target is not None:
            manual_frames += 1
            arm_spd = bb.readDebugParameter('arm_Speed')
            scaled_max = max(1, int(manual_frame_max / arm_spd))
            frac = manual_frames / scaled_max
            if frac >= 1.0:
        ...
        if new_grip_close > old_grip_close_btn:
            close_grip()
            # Only grip cube if end-effector is close enough to it
            try:
                ep, _ = bb.getLinkPose(robot, ee_link)
                dx_c = ep[0]-cube_pos[0]; dy_c = ep[1]-cube_pos[1]; dz_c = ep[2]-cube_pos[2]
                if math.sqrt(dx_c*dx_c + dy_c*dy_c + dz_c*dz_c) < 0.15:
                    gripped = True
            except:
                pass
    ...
    if gripped:
        try:
            ...
            carry_pos = [ep[0], ep[1], ep[2]-CARRY]
            bb.resetBasePose(cube, carry_pos, eo)
            cube_pos[:] = carry_pos
        except: pass
